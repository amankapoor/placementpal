...
func NewMGO(url string, timeout time.Duration) (*DB, error) {
	...
	usersStudentsIndex := mgo.Index{
		Name:       "users_students_index",
		Key:        []string{"google_id", "email"},
		Unique:     true,
		DropDups:   true,
		Background: true,
		Sparse:     true,
	...
	db.database.C("master_data").EnsureIndex(masterDataIndex)
	db.database.C("users_students").EnsureIndex(usersStudentsIndex)
